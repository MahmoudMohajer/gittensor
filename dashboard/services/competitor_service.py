...
def fetch_gittensor_prs(token, db_conn, lang_weights, repo_weights):
    ...
    print("Searching for PRs with Gittensor tagline...")
        ...
        try:
            ...
            for item in items:
                ...
                for rf in raw_files:
                    try:
                        file_changes.append(
                            FileChange.from_github_response(pr_number, repo, rf)
                        )
                ...
                estimated_score = pr.base_score = pr.repo_weight_multiplier = (
                ...
                try:
                    api_resp = requests.post(
                        ...
                        json={
                            ...
                            "github_token": token,
                            "total_open_prs": 0,
                            "assume_merged": True,  # allow scoring before merge
                        },
                        timeout=20,
                    )
                    if api_resp.status_code == 200:
                        ...
                        pr.repo_weight_multiplier = pr_data.get("multipliers", {}).get(
                            "repo_weight_multiplier", 1.0
                        )
                        pr.gittensor_tag_multiplier = pr_data.get(
                            "multipliers", {}
                        ).get("gittensor_tag_multiplier", 1.0)
                        estimated_score = score_data.get(
                            "earned", pr.calculate_final_earned_score()
                        )
                    else:
                        print(
                            f"Scoring API error {api_resp.status_code}: {api_resp.text}"
                        )
                ...
                print(
                    f"  {pr_id}: +{additions}/-{deletions} -> ~{estimated_score:.2f} pts"
                )
